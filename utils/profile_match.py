...
def peilperprofiel(cur, peilcriterium="min", debug=0):
    ...
    if peilcriterium != 'min' and peilcriterium != 'max':
        peilcriterium = 'min'
        if debug:
            print "PEILCRITERIUM AANGEPAST NAAR %s" % peilcriterium
    ...
    cur.execute(q)
    if debug:
        print "aantal gemeten profielen in een hydro_object met een peil: %d " % len(prof)
    return prof
...
def haal_meetprofielen1(cur, profielsoort="Z1", peilcriterium="min", debug=0):
    ...
    for proid in peilvanprofiel:
        #print proid, peilvanprofiel[proid]
    ...
    if debug:
        print 'aantal profielen in hydro-objecten met een peil: %d' % len(prof)
    return prof
...
def projecteerprofielen(prof, projectie="eindpunt", debug=0):
    ...
    if projectie == 'eindpunt':
        ...
        if debug:
            print "PROJECTIECRITERIUM NIET GELDIG %s, GEEN PROJECTIE!" % projectie
    return prof
...
def prof_in_prof(profgem, proftheo, aantstap=100, delta=0.001, obdiepte=0.3, debug=0):
    ...
    if debug:
        print "wb_gem: %.2f, wb_theo: %.2f; stap: %.2f, zoekopp: %.3f" % \
    ...
    for i in xrange(aantstap):
        ...
        if abs(zoekopp-inter.area) < delta:   #  opp intersectie == zoekopp dus past profzoek volledig in gemprof
            ...
            traject += '1'
            if debug:
                print 'Volledig: optimaal: %f; maxopp: %f' % (optimaal, maxopp)
        else:
            if inter.area == maxopp:
                ...
                traject += '2'
                if debug:
                    print 'Niet vol: optimaal: %f; maxopp: %f' % (optimaal, maxopp)
            else:
                traject += '0'
    ...
    if debug:
        print 'Na loop: Fit: %.3f; optimaal: %f' % (fit, optimaal)
        print traject
        ...
        try:
            ...
            try:
                if len(hlijn) == 2:  # is hlijn een MultiLineString van twee LineStrings
                    ...
                    if debug:
                        print "hlijn 3 stuks, xl, xr, hl", xlinks, xrechts, hlijn
...
def doe_profinprof(cur0, cur1, aantalstappen=200, precisie=0.0001, codevastebodem="Z1", peilcriterium="min",
    ...
    try:
        ...
        verrijkgemprof(cur0, gemetenprofielen)
...
con = sql.connect('../tests/data/test_spatialite_with_theoretical_profiles_mp_20171206.sqlite')
cur0 = con.cursor()
cur1 = con.cursor()
